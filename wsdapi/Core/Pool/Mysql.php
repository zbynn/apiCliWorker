...
use Core\Db\Mysql as DB;
use chan;

require_once COREPATH . DS . 'Db/Mysql.php';
...
class Mysql implements PoolInterface
{
    ...
    public function put($mysql)
    {
        if ($this->getLength() >= $this->config['pool_size']) {
            throw new \PDOException("pool full");
        }
    ...
    public function get()
    {
        ...
        if (false === $mysql) {
            throw new \PDOException("get mysql timeout, all mysql connection is used");
        }
        return $mysql;
    }
